...
import { Loader2, ChevronUp, Eye, Check, ChevronDown } from 'lucide-react'
import { sectionGroups } from './DirectorySidebar'
        ...
        <div className="px-4 lg:px-6 py-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              <div>
                <h1 className="text-xl font-semibold text-gray-900">Directory Listing</h1>
                {lastSaved && (
              ...
              <div className="hidden sm:flex items-center gap-2">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 ${
                    discoverable ? 'bg-emerald-500' : 'bg-gray-200'
                  }`}
                >
                  <span
          ...
          {/* Listing URL (when discoverable) */}
          {discoverable && tenantSlug && (
            <div className="mt-3 pt-3 border-t border-gray-100">
              <div className="flex items-center gap-2 text-sm">
                <span className="text-gray-500">Your listing:</span>
                <a
                  href={`/accommodation/${tenantSlug}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-emerald-600 hover:underline"
                >
                  /accommodation/{tenantSlug}
                </a>
              </div>
            </div>
          )}
